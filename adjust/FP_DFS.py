...
png(file.path(output_dir, "forestplot_DFS.png"), width=800, height=600)
library(survival)
library(forestplot)
library(grid)
...
### ---------------------------
model_all <- coxph(Surv(DFSMONTHS, DFS_event) ~ STUDY, data = r_df)
...
for (subgroup in names(subgroup_map)) {
  ...
  # ヘッダー行（isHeader = TRUE、Count0, Count1, Nは空）
  ...
  for (lev in levels_sub) {
    ...
    # STUDY の水準が 2 種類以上（0 と 1 両方がある）場合のみ解析
    if(nrow(subset_data) > 0 && length(unique(subset_data$STUDY)) > 1) {
      model <- coxph(Surv(DFSMONTHS, DFS_event) ~ STUDY, data = subset_data)
      summ <- summary(model)
...
### フォレストプロット用テーブルの作成
### ---------------------------
# ラベルの作成： "All patients" はそのまま、ヘッダー行はサブグループ名、その他はインデントして表示
results$Label <- ifelse(results$Subgroup=="All patients", "All patients",
                        ifelse(results$isHeader, results$Subgroup, paste0("   ", results$Level)))
tabletext <- rbind(c("Subgroup", "ADJUST\nATEZO+CDDP+VNR\n", "IMPACT\nCDDP+VNR\n", "HR (95% CI)\n"),
                   cbind(results$Label, results$Count0, results$Count1,
                         ifelse(is.na(results$HR), "", sprintf("%.2f (%.2f - %.2f)", results$HR, results$lower, results$upper))))
tabletext <- as.matrix(tabletext)

### ---------------------------
### 箱サイズ（point size）の設定：症例数 (N) に比例
maxN <- max(results$N, na.rm=TRUE)
boxsize_vals <- ifelse(is.na(results$N), NA, 0.1 + 0.2*(results$N / maxN))
boxsize_vec <- c(NA, boxsize_vals)

### ---------------------------
### is.summary ベクトルの作成：
# ヘッダー行（テーブルの1行目）は FALSE,
# 以降、「All patients」の行は TRUE（ひし形で表示）、それ以外は FALSE
is_summary_vec <- c(FALSE, results$Subgroup == "All patients")

### ---------------------------
### 背景色の設定（すべて白）
nrows <- nrow(tabletext)
grid.newpage()
for(i in 1:nrows) {
  y_pos <- 1 - (i - 0.5) / nrows
  grid.rect(x = 0.5, y = y_pos, width = 1, height = 1/nrows, gp = gpar(fill = "#FFFFFF", col = NA))
}

### ---------------------------
### フォレストプロットの描画
### ---------------------------
myfp <- forestplot(labeltext = tabletext,
                   ...
                   xlab = "Hazard Ratio",
                   title = "",
                   txt_gp = fpTxtGp(label = gpar(fontfamily = "", cex = 0.9),
                                    ticks = gpar(cex = 0.8),
                                    xlab = gpar(cex = 0.9)),
...
# タイトルを下げて描画
grid.text("Forest Plot of Integrated Analysis Outcome According to Subgroups", 
          x = unit(0.5, "npc"),   # 横の位置（中央）
          y = unit(0.9, "npc"),  # 縦の位置（もっと下にするなら 0.8 など）
          gp = gpar(cex = 1.2))   # 文字の大きさを調整
                   
print(myfp)

### ---------------------------
### HR軸下部に矢印とテキストを追加（NEJM Figure 4 のように）
### ---------------------------
grid.segments(x0 = unit(0.63, "npc"), y0 = unit(0.12, "npc"),
              x1 = unit(0.9, "npc"), y1 = unit(0.12, "npc"),
              arrow = arrow(length = unit(0.25, "cm"), ends = "both", type = "closed"),
              gp = gpar(col = "black", lwd = 2))
grid.text("Favors ADJUST", x = unit(0.63, "npc"), y = unit(0.1, "npc"), just = "center", gp = gpar(col = "black", cex = 0.9))
grid.text("Favors IMPACT", x = unit(0.9, "npc"), y = unit(0.1, "npc"), just = "center", gp = gpar(col = "black", cex = 0.9))
